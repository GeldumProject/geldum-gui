...
{
    // prepare command line arguments and pass to geldumd
...
//    arguments << "--check-updates" << "disabled";
    ...
    qDebug() << "starting geldumd " + m_geldumd;
    ...
    // Start geldumd
    bool started = m_daemon->startDetached(m_geldumd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM geldumd.exe");
#else
                QProcess::execute("pkill geldumd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./geldumd status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_geldumd, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to geldumd
#ifdef Q_OS_WIN
    m_geldumd = QApplication::applicationDirPath() + "/geldumd.exe";
#elif defined(Q_OS_UNIX)
    m_geldumd = QApplication::applicationDirPath() + "/geldumd";
#endif

    if (m_geldumd.length() == 0) {
