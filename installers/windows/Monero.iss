...
[Setup]
AppName=Geldum GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.1.0.0
DefaultDirName={pf}\Geldum GUI Wallet
DefaultGroupName=Geldum GUI Wallet
UninstallDisplayIcon={app}\geldum-wallet-gui.exe
...
; Geldum GUI wallet
Source: "bin\geldum-wallet-gui.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Geldum GUI wallet log file
...
Source: "geldum-wallet-gui.log"; DestDir: "{app}"; Flags: comparetimestamp; Permissions: users-modify

; Geldum CLI wallet
Source: "bin\geldum-wallet-cli.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Geldum wallet RPC interface implementation
Source: "bin\geldum-wallet-rpc.exe"; DestDir: "{app}"; Flags: comparetimestamp
...
Source: "bin\geldum-blockchain-import.exe"; DestDir: "{app}"; Flags: comparetimestamp

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\geldum-utils-deserialize.exe"; DestDir: "{app}"; Flags: comparetimestamp
...
; Filename: "{app}\geldum-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\geldum');
  s := 'The default folder to store the Geldum blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\geldum.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "geldum-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Geldum daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\geldumd.exe') + '" ' + DaemonFlags('--detach');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\geldum-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Geldum GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\geldum-wallet-gui.exe"
...
Name: "{group}\Utilities\Geldum Daemon"; Filename: "{app}\geldumd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\geldum-wallet-cli.exe"; WorkingDir: "{userdocs}\Geldum\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Geldum\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\geldum-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\geldum-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM geldumd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\geldum-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\geldum-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\geldum-project\geldum-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\geldum-project\geldum-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
